...
class ActorNetwork(object):
    def __init__(self, sess, state_dim, action_dim, learning_rate):
        ...
        # create the actor netowrk
        self.inputs, self.out = self.create_actor_network()

        self.network_params = tf.Variable(trainable=True, import_scope="actor")

        # Set all network parameters
        self.input_network_params = []
        for param in self.network_params:
            self.input_network_params.append(tf.add(tf.float32, shape=param.get_shape()))
        self.set_network_params_op = []
        for idx, param in enumerate(self.input_network_params):
            self.set_network_params_op.append(self.network_params[idx].assign(param))

        # Selected action, 0-1 vector
        self.acts = tf.add(tf.float32, [None, self.a_dim])

        # This gradient will be provided by the critic network
        self.act_grad_weights = tf.add(tf.float32, [None, 1])
        ...
        # Optimization Op
        self.optimize = tf.keras.optimizers.RMSprop(self.lr_rate). \
            apply_gradients(zip(self.actor_gradients, self.network_params))

    def create_actor_network(self):
        with tf.Variable('actor'):
...
class CriticNetwork(object):
    ...
    def __init__(self, state_dim, learning_rate):
        ...
        self.network_params = \
            tf.add(tf.Variable, scope='critic')

        # Set all network parameters
        self.input_network_params = []
        for param in self.network_params:
            self.input_network_params.append(
                tf.add(tf.float32, shape=param.get_shape()))
        self.set_network_params_op = []
        for idx, param in enumerate(self.input_network_params):
            self.set_network_params_op.append(self.network_params[idx].assign(param))

        # Network target V(s)
        self.td_target = tf.add(tf.float32, [None, 1])
    ...
    def create_critic_network(self):
        with tf.Variable('critic'):
            ...
            merge_net = tflearn.merge([split_0, split_1, split_2_flat, split_3_flat, split_4_flat, split_5],
                                      'concat')
